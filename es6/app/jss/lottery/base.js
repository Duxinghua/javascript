...
class Base{
	...
	addCodeItem(code,type,typeName,count){
		...
		<li codes ="${type}|${code}" bonus="${count*2}" count="${count}">
			...
			<b class="em">${code}</b>
			[${count}注,<em class="code-list-mon">${count*2}</em>元]
			</div>
		</li>
	...
	getCount(){
		...
		let count = self.computeCount(active,self.cur_play);
		let range = self.computeBouns(active,self.cur_play);
		let money = count * 2;
		...
		if(count === 0){
			...
			tpl = `您选了<b>${count}</b>注，共<b>${count*2}</b>元<em>若中奖，奖金:
			<strong class="red">${range[0]}</strong>元到<strong class="red">${range[1]}</strong>元,
			您将${win1 < 0 && win2 < 0 ?'亏损':'盈利'}
			<strong class="${win1 >=0 ? 'red' :'green'}">${c1}</strong>元 至
	...
	getRandom(num){
		...
		while(num--){
			index = Number.pareInt(Math.random()*number.length);
